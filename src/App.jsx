...
import { Book, Loader2, ClipboardCheck, Library, MessageSquare, Download, Printer } from 'lucide-react';
...
export default function App() {
  ...
  // Function to call the backend API proxy
  const generateCatalogueRecord = async (prompt) => {
    // Check if we're in development mode (localhost)
    const isDevelopment = window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1';
    
    if (!isDevelopment) {
      throw new Error('This feature requires a local backend server. Please see the README for setup instructions.');
    }
...
${itemInput}`;
  ...
  // Handler for the copy button click using modern Clipboard API
  const handleCopy = async () => {
    if (catalogueOutput) {
      try {
        await navigator.clipboard.writeText(catalogueOutput);
        setIsCopied(true);
    ...
    <div className="bg-[#E2F2F2] min-h-screen p-4 sm:p-8 flex items-center justify-center font-sans text-gray-800">
      <div className="bg-white rounded-xl shadow-2xl p-6 sm:p-10 w-full max-w-2xl">
        ...
        <div className="flex items-center space-x-4 mb-8 border-b-2 border-teal-700 pb-4">
          <Library className="w-12 h-12 text-[#007582]" />
            ...
            {window.location.hostname !== 'localhost' && window.location.hostname !== '127.0.0.1' && (
              <div className="mb-6 p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
                <p className="text-sm text-yellow-800">
                  <strong>Note:</strong> The AI features require a local backend server with a Gemini API key. 
                  To use this functionality, please clone the repository and follow the setup instructions in the README.
                </p>
              </div>
            )}
            <div className="mb-6">
